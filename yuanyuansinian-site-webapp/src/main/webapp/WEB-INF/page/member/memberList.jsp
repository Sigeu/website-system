...
<body>
	<nav class="breadcrumb">
		...
		系统管理 <span class="c-gray en">&gt;</span>会员管理
	</nav>
	<div id="win"></div>
	<div id="win2"></div>
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			var oTable = $("#data_table")
					.DataTable(
							{
								ajax : {
									url : "${pageContext.request.contextPath}/sinian/member/memberController/queryMemberList",
									type:"POST",
									data : {
										//args1: "固定传参"
									}
			...
			$('#add_but').on('click', function() {
				layer.open({
				    ...
				    content: '${pageContext.request.contextPath}/sinian/member/memberController/toMemberAdd'
				 });
			});
		...
		function toEdit(id){
			layer.open({
			    ...
			    area: ['100%', '100%'],
			    shadeClose: false, //点击遮罩关闭
			    content: '${pageContext.request.contextPath}/sinian/member/memberController/toMemberUpdate?id='+id
			 });
		}
		//删除
		function toDelete(id){
			layer.confirm("确认删除？", {
				  btn: ['确认','返回'] //按钮
						...
						$.ajax({
						    url: "${pageContext.request.contextPath}/sinian/member/memberController/deleteMember" ,
						    type: "POST",
						    dataType: "JSON",
						    data: {id:id},
		...
		function toDetail(id){
			layer.open({
			    ...
			    area: ['100%', '100%'],
			    shadeClose: false, //点击遮罩关闭
			    content: '${pageContext.request.contextPath}/sinian/member/memberController/toMemberDetail?id='+id
			 });
		}
