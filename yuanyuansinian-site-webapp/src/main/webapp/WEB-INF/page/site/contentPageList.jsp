...
$(function() {
	$.ajax({
		method : "POST",
		url : contextPath + "/sinian/index/indexController/contentForPage",
		data : {
			id : '${column_id }'
		}
	}).done(function(data) {
	    ...
	    "callBack":function(result){
	        ...
	        for(var i=0;i<result.length;i++){
	            cHtml += "<div class='search-pro'><a href='###'  onclick=\"toContentDetail('" + result[i].id +"')\"><h5>"+ result[i].title + "</h5></a><p class='search-date'>" + result[i].add_time.substring(0,10) + "</p></div>";//处理数据
	        }
