...
$(function() {
	$.ajax({
		method : "POST",
		url : contextPath + "/sinian/index/indexController/cemeteryForPage",
		data : {
			id : '${cemetery_type }'
		}
	}).done(function(data) {
	    ...
	    "callBack":function(result){
	        ...
	        for(var i=0;i<result.length;i++){
	            cHtml += "<div class='search-pro'><a href='###'  onclick=\"toCemeteryDetail('" + result[i].id +"')\"><h5>"+ result[i].title + "</h5></a><p class='search-date'>" + result[i].create_date.substring(0,10) + "</p></div>";//处理数据
	        }
