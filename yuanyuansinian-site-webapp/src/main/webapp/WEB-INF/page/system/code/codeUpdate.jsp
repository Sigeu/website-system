...
<body class="pos-r">
		<nav class="breadcrumb">
			首页 <span class="c-gray en">&gt;</span> 系统管理 <span class="c-gray en">&gt;</span>字典项修改
		</nav>
		<div class="page-container">
		<form id="form_"
			action="${pageContext.request.contextPath}/system/controller/codeController/updateCode" class="form form-horizontal" 
			method="post">
			...
			<table style="width: 99%" class="table table-border table-bordered">
				<tr>
					...
					<td nowrap="nowrap"><input type="text" style="width: 90%;" placeholder="字典名称"
						id="code_name" name="code_name" value="${code.code_name }" class="input-text" maxlength="25"
						 /> <font style="color: red">*</font></td>

					<td align="right" width="16%" nowrap="nowrap" class="text-r mybg">字典值：</td>
					<td nowrap="nowrap">
					<input type="text"  style="width: 90%;"  placeholder="字典项数值"
						id="code_value" name="code_value" value="${code.code_value }" class="input-text" maxlength="25" />
				...
				<tr>
					...
					<td nowrap="nowrap"><input type="text"  style="width: 90%;"  placeholder="排序值，数值越小越靠前"
						id="order_num" name="order_num" value="${code.order_num }" class="input-text" />
					</td>
				...
				<tr>
					...
					<td colspan="3"><textarea style="width: 90%;" id="remark" placeholder="备注说明，最多300个汉字"
							name="remark" rows="2" class="textarea" maxlength="600">${code.remark }</textarea>
					</td>
				</tr>
			</table>
...
$(function(){
	...
	//设置选中
	//$("input:radio[name='is_edit']").val('${codeType.is_edit }');
	$("input[name='status'][value='${codeType.status }']").attr("checked",true); 
});
