...
public class UserRealm extends AuthorizingRealm {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(
		...
		if(null != username){
			// 查询用户授权信息
			List<RoleFuncright> roleFuncrightList = funcrightService.queryUserFuncright(username);
			if (null != roleFuncrightList && !roleFuncrightList.isEmpty()) {
				info = new SimpleAuthorizationInfo();
				for (RoleFuncright roleFuncright : roleFuncrightList){
					System.out.println("000000000000000000000" + roleFuncright.getFuncright_code());
	...
	protected AuthenticationInfo doGetAuthenticationInfo(
		...
		if (null != user) {
			 authenticationInfo = new SimpleAuthenticationInfo(user.getLogin_name(), user.getPassword(), getName());
