...
public class DataTablePageUtil<T> {
	 ...
	 */
	private int start = ISystemConstants.PAGE_START;// 起止位置
	/*
	 * 告诉服务器每页显示的条数，这个数字会等于返回的 data集合的记录数，可能会大于因为服务器可能没有那么多数据。
	 * 这个也可能是-1，代表需要返回全部数据(尽管这个和服务器处理的理念有点违背)
	 */
	private int length = ISystemConstants.PAGE_LENGTH; // 数据长度
	 ...
	 * 当前页码
	 */
	private int page_num =ISystemConstants.PAGE_NUM;
	
	/*
	 * 每页数据
	 */
	private int page_size = ISystemConstants.PAGE_SIZE;
