...
public class MyIpV4Util {
	...
	/**
	 * @param mask
	 *            任意的如"255.255.254.0"等格式，如果格式不合法，抛出UnknownError异常错误
	 */
	public MyIpV4Util(String masks) {
	...
	/**
	 * 比较两个ip地址是否在同一个网段中，如果两个都是合法地址，两个都是非法地址时，可以正常比较； 如果有其一不是合法地址则返回false；
	...
	/**
	 * 获取默认mask值，如果IpV4是A类地址，则返回{@linkplain #DefaultIpAMask}， 如果IpV4是B类地址，则返回
	 * {@linkplain #DefaultIpBMask}，以此类推
	 * 
	 * @param anyIpV4
	 *            任何合法的IpV4
	 * @return mask 32bit值
	...
	/**
	 * 获取默认mask地址，A类地址对应255.0.0.0，B类地址对应255.255.0.0， C类及其它对应255.255.255.0
	 * 
	 * @param anyIp
	 * @return mask 字符串表示
	 */
	...
	public static String trans2IpStr(int ipValue) {
		// 保证每一位地址都是正整数
		return ((ipValue >> 24) & 0xff) + "." + ((ipValue >> 16) & 0xff) + "."
				+ ((ipValue >> 8) & 0xff) + "." + (ipValue & 0xff);
	}
	...
	public static String trans2IpV4Str(byte[] ipBytes) {
		// 保证每一位地址都是正整数
		return (ipBytes[0] & 0xff) + "." + (ipBytes[1] & 0xff) + "."
	...
	public static void main(String[] args) throws UnknownHostException {
		...
		System.out.println("ip1和ip2在同一个网段中？ "
				+ (checkSameSegment(ip1, ip2, mask1)));

		System.out.println("ip3和ip4在同一个网段中 ？"
				+ (checkSameSegment(ip3, ip4, mask2)));
		...
		if (checkSameSegment(ip1, ip5, mask1)) {
			// 判断ip5是否在ip1和ip2范围中
			if (((compareIpV4s(ip5, ip1)) >= 0)
					&& (compareIpV4s(ip5, ip2) <= 0)) {
