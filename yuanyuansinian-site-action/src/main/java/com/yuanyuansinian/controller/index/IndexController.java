...
import com.yuanyuansinian.tenpay.AccessTokenRequestHandler;
import com.yuanyuansinian.tenpay.ClientRequestHandler;
import com.yuanyuansinian.tenpay.PackageRequestHandler;
import com.yuanyuansinian.tenpay.PrepayIdRequestHandler;
import com.yuanyuansinian.tenpay.util.ConstantUtil;
import com.yuanyuansinian.tenpay.util.TenpayUtil;
import com.yuanyuansinian.tenpay.util.WXUtil;
...
public class IndexController extends MyBaseController {
	...
	public void toTenpay(HttpServletRequest request, HttpServletResponse response, Model model) {
		try {
			...
			if (!"".equals(token)) {
				...
				packageReqHandler.setParameter("total_fee", totalAmount+""); //商品金额,以分为单位  
			...
			sb.append("<retmsg>" + retmsg + "<retmsg>");
			if (!"".equals(xml_body)) {
				sb.append(xml_body);
			}
			sb.append("</root>");
			...
			response.setContentType("text/html;charset=" + IMySystemConstants.CHARSET);
			response.getWriter().write(sb.toString());// 直接将完整的表单html输出到页面
			response.getWriter().flush();
			response.getWriter().close();
		} catch (Exception e) {
			// TODO: handle exception
